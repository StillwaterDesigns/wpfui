...
public partial class NavigationView : System.Windows.Controls.Control, INavigationView {
	...
	protected static void UpdateVisualState(NavigationView navigationView) {
		...
		if (navigationView.PaneDisplayMode
		...
		_ = VisualStateManager.GoToState(navigationView,
	...
	/// </summary>
	private void AutoSuggestBoxOnSuggestionChosen(AutoSuggestBox sender,
	...
	private void AutoSuggestBoxOnQuerySubmitted(AutoSuggestBox sender,
	...
	protected virtual void AddItemsToDictionaries(IEnumerable list) {
		foreach (NavigationViewItem singleNavigationViewItem in list.OfType<NavigationViewItem>()) {
			if (!PageIdOrTargetTagNavigationViewsDictionary.ContainsKey(singleNavigationViewItem.Id)) {
				PageIdOrTargetTagNavigationViewsDictionary.Add(singleNavigationViewItem.Id,
					singleNavigationViewItem);
			}

			if (!PageIdOrTargetTagNavigationViewsDictionary
					.ContainsKey(singleNavigationViewItem.TargetPageTag)) {
			...
			if (singleNavigationViewItem.TargetPageType is not null
					...
					.ContainsKey(singleNavigationViewItem.TargetPageType)) {
